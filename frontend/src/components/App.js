...
function App() {
  ...
  function handleCardLike(card) {
        ...
        const isLiked = card.likes && card.likes.length > 0 && card.likes.some((i) => i._id === currentUser._id);
        api
          .changeLikeCardStatus(!isLiked, "cards/", card._id)
          .then((newCard) => {
  ...
  function tokenCheck() { //esta funcion comprueba el token para la auteticacion en automatico para no este logiando siempre 
    console.log("datos del token en local storag");
